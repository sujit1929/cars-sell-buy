...
const SectionTitle = ({
  ...
  <div className="mb-8">
    ...
    {subtitle && (
      <p className="text-slate-600 max-w-2xl text-sm md:text-base font-light">
        {subtitle}
      </p>
    )}
  </div>
  ...
  >
    ...
    <div className="p-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <Button className="w-full text-xs h-9 bg-black hover:bg-slate-800 text-white">
          Details
        </Button>
      </div>
    </div>
    ...
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled || isMobileMenuOpen
          ...
          : "bg-linear-to-b from-black/80 to-transparent text-white py-6"
      }`}
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        <div className="flex items-center gap-2 cursor-pointer z-50">
          ...
          <span className="font-serif tracking-widest text-lg hidden sm:block">
            MERCEDES-BENZ
          </span>
        </div>
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-sm font-medium hover:text-[#00adef] transition-colors"
            >
              {link.label}
            </a>
          ))}
      ...
      <div className="absolute inset-0 flex flex-col justify-end pb-24 px-6 container mx-auto z-10">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Button variant="default" className="bg-[#00adef] hover:bg-[#0090c5]">
              Configure Now
            </Button>
          </div>
        </motion.div>
...
const ModelsSection = () => {
  ...
  const recentCars = CAR_DATA.filter(
    (car) =>
      car.tags?.includes("New Arrival") || car.id === "4" || car.id === "1"
  );
  // 2. Filter for "Top Bought"
  const topCars = CAR_DATA.filter(
    (car) =>
      car.tags?.includes("Top Bought") ||
      car.tags?.includes("Best Seller") ||
      car.category === "SUV"
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="">
          <div className="flex justify-between items-end mb-8 border-b border-gray-200 pb-4">
            ...
            <a
              href="#"
              className="hidden md:flex items-center gap-2 text-sm font-semibold hover:text-[#00adef] mb-2"
            >
        ...
        <section id="models" className="py-20 bg-white">
          <div className="container mx-auto px-6">
            <SectionTitle
              title="Our Models"
              subtitle="Find the perfect vehicle that matches your lifestyle and ambition."
            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              <AnimatePresence mode="popLayout">
                {topCars.map((car) => (
                  ...
                  >
                    <div className="relative overflow-hidden aspect-4/3 mb-4 bg-gray-100 rounded-lg">
                      <Image
                        src={car.image}
                        alt={car.name}
                        fill
                        className="object-cover transition-transform duration-700 group-hover:scale-105"
                      />
                      <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur px-3 py-1 text-xs font-bold uppercase rounded">
                        {car.category}
                      </div>
                    </div>

                    <h3 className="text-xl font-semibold mb-1 group-hover:text-[#00adef] transition-colors">
                      {car.name}
                    </h3>
                    ...
                    <div className="flex items-center gap-6 text-sm text-gray-600 mb-6">
                      <span className="flex items-center gap-1">
                        <Zap className="w-4 h-4" /> {car.power}
                      </span>
                      <span className="flex items-center gap-1">
                        <ArrowRight className="w-4 h-4" /> {car.acceleration}
                      </span>
                    </div>
  ...
  return (
    <section className="bg-neutral-900 text-white py-24">
      <div className="container mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
        <div>
          <span className="text-[#00adef] font-semibold tracking-wider uppercase mb-2 block">
            Innovation
          </span>
...
const DealerLocatorStub = () => {
    return (
        <section className="py-20 bg-gray-50">
            <div className="container mx-auto px-6 text-center">
                <h2 className="text-3xl font-light mb-8">Find a Dealer Near You</h2>
                <div className="max-w-2xl mx-auto bg-white p-2 rounded-lg shadow-lg flex items-center">
                    <MapPin className="ml-4 text-muted-foreground" />
                    <input
                        type="text"
                        placeholder="Enter City or Zip Code"
                        className="flex-1 p-4 outline-none text-gray-700"
                    />
                    <Button variant="secondary" className="md:block hidden">Search</Button>
                </div>
                <div className="mt-8 relative w-full h-64 bg-gray-200 rounded-lg overflow-hidden flex items-center justify-center">
                    <p className="text-muted-foreground flex items-center gap-2">
                        <MapPin /> Map Mockup Interface
                    </p>
                </div>
            </div>
        </section>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className=" border border-muted-foreground rounded-2xl bg-transparent text-sm py-2 px-3 w-full text-white"
              />
              <button className="bg-white text-black px-4 rounded-2xl text-sm font-bold">
                Go
              </button>
            </div>
          </div>
